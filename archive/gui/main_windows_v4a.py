...
def raw_adc_to_pressure_no_zero(raw_adc, sensor_index):
    ...
    with data_store.lock:
        for i in range(config["num_p"]):
            if len(data_store.history_y[i]) > 0:
                raw_adc = data_store.history_y[i][-1]
                psi = raw_adc_to_pressure_no_zero(raw_adc, i)
...
def convert_pressure(raw_adc, sensor_index):
    psi = raw_adc_to_pressure_no_zero(raw_adc, sensor_index)
