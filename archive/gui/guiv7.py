...
It stores sensor/valve counts, relay mapping, and macros for external editing/sharing.
"""
...
class GroundStationWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Liquid Propulsion Ground Station - guiv7")
    ...
    def load_shared_app_config(self) -> AppConfig:
        ...
        return AppConfig(
            ...
            sensor_type=["low"] * max(1, int(payload.get("pressure_sensors", payload.get("num_p", 4)) or 1)),
        )
    ...
    def rebuild_plot_curves(self) -> None:
        ...
        for i in range(self.config.num_p):
            ...
            curve = self.plot_widget.plot([], [], name=f"P-{i + 1}", pen=pg.mkPen(color, width=3))
    ...
    def raw_adc_to_voltage(self, raw_adc: int) -> float:
        return raw_adc * ADS_UNIT_VOLTAGE

    def convert_pressure(self, raw_adc: int, sensor_index: int) -> float:
        # Direct conversion only: ADS1115 counts -> voltage -> 0-5000 psig.
        # Assumes the transducer is 0.5 V at 0 psig and 5.0 V at 5000 psig.
        voltage = self.raw_adc_to_voltage(raw_adc)
        return (voltage - V_MIN) / V_DIFF * PRESSURE_MAX
