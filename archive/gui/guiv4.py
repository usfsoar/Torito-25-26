...
class MacroEditorDialog(QtWidgets.QDialog):
    """Table-based macro editor so users do not have to hand-write JSON."""

    ACTIONS = ["set", "high", "low", "wait", "estop", "send_bits"]
        ...
        "macros": [
            {
                ...
                "steps": [
                    ...
                    {"action": "set", "valve": 2, "state": "high"},
                    {"action": "wait", "seconds": 0.5},
                    {"action": "set", "valve": 2, "state": "low"},
                ],
            }
        ]
    }
    ...
    def __init__(self, parent: QtWidgets.QWidget | None = None):
        ...
        help_text = QtWidgets.QLabel(
            "Rules: macros run with Shift+letter; Shift+A is reserved for E-stop; valve numbers are 1-based; macros must set explicit states, not toggle.\n"
            "Actions: set, high, low, wait, estop, send_bits."
        )
        ...
        self.step_table = QtWidgets.QTableWidget(0, 5)
        self.step_table.setHorizontalHeaderLabels(["Action", "Valve", "State", "Seconds", "Bits"])
    ...
    def _normalize_macro(self, macro: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(steps, list):
            steps = []
        return {"key": key, "name": name, "steps": [dict(s) for s in steps if isinstance(s, dict)]}
    ...
    def _refresh_macro_table(self) -> None:
        ...
        self._loading = False
        if self._macros:
            self.macro_table.selectRow(min(self.macro_table.currentRow(), len(self._macros) - 1) if self.macro_table.currentRow() >= 0 else 0)
    ...
    def _set_step_row_widgets(self, row: int, step: dict[str, Any]) -> None:
        ...
        action_combo.currentTextChanged.connect(lambda _=None, r=row: self._write_step_from_row(r))
        self.step_table.setCellWidget(row, 0, action_combo)

        valve_spin = QtWidgets.QSpinBox()
        valve_spin.setRange(1, 9)
        valve_spin.setValue(int(step.get("valve", 1) or 1))
        valve_spin.valueChanged.connect(lambda _=None, r=row: self._write_step_from_row(r))
        self.step_table.setCellWidget(row, 1, valve_spin)

        state_combo = QtWidgets.QComboBox()
        state_combo.addItems(self.STATES)
        state = str(step.get("state", "high")).lower().strip()
        ...
        self.step_table.setCellWidget(row, 2, state_combo)

        seconds_item = QtWidgets.QTableWidgetItem(str(step.get("seconds", "")))
        bits_item = QtWidgets.QTableWidgetItem(str(step.get("bits", "")))
        self.step_table.setItem(row, 3, seconds_item)
        self.step_table.setItem(row, 4, bits_item)

    def _step_cell_changed(self, row: int, column: int) -> None:
        if not self._loading and column in (3, 4):
    ...
    def _write_step_from_row(self, row: int) -> None:
        ...
        if idx < 0 or row < 0 or row >= self.step_table.rowCount():
            return
        action_widget = self.step_table.cellWidget(row, 0)
        valve_widget = self.step_table.cellWidget(row, 1)
        state_widget = self.step_table.cellWidget(row, 2)
        action = action_widget.currentText() if isinstance(action_widget, QtWidgets.QComboBox) else "set"
        valve = valve_widget.value() if isinstance(valve_widget, QtWidgets.QSpinBox) else 1
        state = state_widget.currentText() if isinstance(state_widget, QtWidgets.QComboBox) else "high"
        seconds_text = self.step_table.item(row, 3).text().strip() if self.step_table.item(row, 3) else ""
        bits_text = self.step_table.item(row, 4).text().strip() if self.step_table.item(row, 4) else ""

        step: dict[str, Any] = {"action": action}
        if action in {"set", "high", "low"}:
            step["valve"] = valve
        if action == "set":
            step["state"] = state
        if action == "wait":
            try:
                step["seconds"] = float(seconds_text) if seconds_text else 0.0
            except ValueError:
                step["seconds"] = 0.0
        if action == "send_bits":
            step["bits"] = bits_text or "0x8000"
        self._macros[idx].setdefault("steps", [])[row] = step

    def add_macro(self) -> None:
        self._macros.append({"key": "B", "name": "New Macro", "steps": [{"action": "set", "valve": 1, "state": "high"}]})
        self._refresh_macro_table()
    ...
    def add_step(self) -> None:
        ...
        self._macros[idx].setdefault("steps", []).append({"action": "set", "valve": 1, "state": "high"})
        self._load_steps_for_selected_macro()
...
class GroundStationWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Liquid Propulsion Ground Station - guiv4")
