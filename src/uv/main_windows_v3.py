...
def convert_pressure(raw_adc, sensor_index):
    """
    Replicates Teensy high/low pressure calibration exactly.
    raw_adc: signed int16 from ADS1115
    """

    voltage = raw_adc * ADS_LSB

    sensor_type = config["pressure_types"][sensor_index]
    ...
    if sensor_type == "low":
        ...
        PSI_MAX = 2000.0  # default safe fallback

    psi = (voltage - V_MIN) * (PSI_MAX / (V_MAX - V_MIN))

    # Clamp exactly like Teensy
    if psi < 0.0:
        psi = 0.0
    if psi > PSI_MAX:
        psi = PSI_MAX
        ...
        def update_category(cat_prefix, start_idx, count):
            ...
            for i in range(count):
                ...
                if len(data_store.history_y[global_idx]) > 0:
                    ...
                    if cat_prefix == "P":
                        raw_centipsi = y_data[-1]
                        psi = raw_centipsi / 10.0
